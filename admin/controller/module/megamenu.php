	...
	public function index() {  
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if(isset($_POST['active-module'])){
				if($this->request->post['megamenu_modules'] == 0) $this->response->redirect($this->url->link('module/megamenu', 'token=' . $this->session->data['token'], true));
				$this->response->redirect($this->url->link('module/megamenu&module_id=' . $this->request->post['megamenu_modules'], 'token=' . $this->session->data['token'], true));
		    }
		}
		...
		if(isset($_GET['delete'])) {
			...
			$this->response->redirect($this->url->link('module/megamenu' . $url, 'token=' . $this->session->data['token'], true));
		}
		...
		$data['action'] = $this->url->link('module/megamenu' . $url, 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			'text' => 'MegaMenu',
			'href' => $this->url->link('module/megamenu', 'token=' . $this->session->data['token'], true)
		);
